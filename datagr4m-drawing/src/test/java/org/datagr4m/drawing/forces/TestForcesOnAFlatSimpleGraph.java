package org.datagr4m.drawing.forces;

import java.util.Collection;

import org.apache.log4j.Logger;
import org.datagr4m.drawing.AbstractLayoutRunnerTest;
...
topo.toConsole();
    ...
    public void testForces() {
        ...
        forcesToConsole("repulsor: ", forcesD1);
        ...
        forcesToConsole("repulsor: ", forcesD2);
        ...
        forcesToConsole("repulsor: ", forcesD3);
        ...
        forcesToConsole("repulsor: ", forcesD4);
...
// !!!!!!!!!!!!!!!!!!!
Logger.getLogger(TestForcesOnAFlatSimpleGraph.class).warn("TESTS AS COMMENT!!!");
//!!!!!!!!!!!!!!!!!!!


        /*Collection<IForce> attrD1 = model.getAttractors(d1);
        forcesToConsole("attractor: ", attrD1);
        
        System.out.println(attrD1.size());
        ...
        assertTrue("D2 has no attractor", model.getAttractors(d2).size() == 0);
        assertTrue("D3 has no attractor", model.getAttractors(d3).size() == 0);
        assertTrue("D4 has no attractor", model.getAttractors(d4).size() == 0);
    }

    public void forcesToConsole(String header, Collection<IForce> forces) {
        if (console) {
            for (IForce force : forces)
                System.out.println(header + force.getOwner() + " > " + force.getSource());
        }
    }    
}
