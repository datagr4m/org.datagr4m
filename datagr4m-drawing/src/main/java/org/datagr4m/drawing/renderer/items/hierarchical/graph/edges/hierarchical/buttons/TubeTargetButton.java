...
public class TubeTargetButton extends TubeOpenCloseButton{

    public TubeTargetButton(Tube tube, Point2D position, float radius, String name1, TubeRendererSettings settings) {
        super(tube, position, radius, name1);
        this.settings = settings;
        if(settings.isTubeTargetOpened(tube))
            open();
        else
            close();
    }
	...
	public List<IClickableItem> hit(int x, int y){
        if(circle.contains(x, y)){
            toggleOpenStatus();
            settings.setTubeTargetOpened(tube, isOpen());
