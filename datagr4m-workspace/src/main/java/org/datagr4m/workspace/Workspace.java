...
public class Workspace implements Serializable, IWorkspace{
    ...
    protected IRenderingPolicy renderingPolicy;// = new AbstractRenderingPolicy();
    ...
    public Workspace(Topology<?,?> topology, IHierarchicalModel model, IHierarchicalEdgeModel tubeModel, AnnotationModel amodel, IHierarchicalLayout layout, Map<String,Object> metadata) {
    	this();
	...
	public void initializeRenderer(IDisplay display){
        pluginRenderer = new PluginLayeredRenderer(display, model, tubeModel, annotationModel);
    ...
    public static <V,E> IWorkspace build(Topology<V,E> topology, IHierarchicalModelFactory factory, HierarchicalLayoutFactory layoutFactory, GroupFilter<V> filter, Layout layoutML, Map<String,Object> metadata){
        // main model (including tubes)
        IHierarchicalGraphModel model = (IHierarchicalGraphModel)factory.getLayoutModel(topology); // with tubes
        ...
        IHierarchicalLayout layout = layoutFactory.getLayout(model, model.getEdgeModel()); // with tube layout
    ...
    /**
     * Extract from XML layout settings the type of layout required for each model.
    ...
    /**
     * Read parameters that are specific to a given layout, find this layout, and setup
     * its parameters.
     */
    ...
    // topology is just given to give the ability to the caller to indicate the actual V/E types
	...
	public void applyMap(Map<String,Coord2d> map){
        IHierarchicalModel model = getLayoutModel();
        if(model!=null){
